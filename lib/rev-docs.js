...
function revProjVersionFiles (project, ver) {
  ...
  fs.copySync(`${projDocsDir}/${ver}/project-versions/${project}-${ver}.json`, `${revIndexFolder}/${project}-${ver}.json`)
  ...
  glob.sync(`${projVerDir}/**/*.json`)
        ...
        .map(f => {
          ...
          projVerRevContent.meta[singularize(fileObjType)][entityName] = getFileName(revFileName).replace('.json', '')
        })
