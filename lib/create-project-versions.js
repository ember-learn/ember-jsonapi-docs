...
module.exports = function createProjectVersions(versions, projectName) {
				...
				relationships: {
					classes: {
						data: jsonapidoc.data
							...
							.map(item => ({ id: item.id, type: 'class' })),
					},
					modules: {
						data: jsonapidoc.data
							.filter(item => item.type === 'module')
							.map(item => ({ id: item.id, type: 'module' })),
					},
					namespaces: {
						data: jsonapidoc.data
							.filter(item => item.type === 'class' && item.static === 1)
							.map(item => ({ id: item.id, type: 'namespace' })),
					},
					project: {
						data: {
							id: projectName,
