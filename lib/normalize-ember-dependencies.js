...
module.exports = giantDocument => {
	return new Promise(resolve => {
		...
		nonEmberDocs.forEach(document => {
			_.forIn(document.relationships, (relationship, relationshipName) => {
				if (Array.isArray(relationship.data)) {
					...
					document.relationships[relationshipName].data = fixEmberRelationship(
						relationship.data,
						document.attributes.version
					)
				}
			})
		})
		...
		let missing = nonEmberDocs.map(document => {
			...
			_.forIn(document.relationships, relationship => {
				if (Array.isArray(relationship.data)) {
					missing = missing
